...
import {
  LayoutDashboard, ShoppingCart, Pill, Package, CreditCard,
  ArrowLeftRight, Vault, ClipboardList, BarChart3,
  ScrollText, AlertTriangle, Wallet, Settings, BookUser
} from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { href: "/medicines",      label: "إدارة الأصناف",  icon: Pill },
  ...
  { href: "/shifts",         label: "خزائن البائعين",  icon: Vault },
...
export function AppSidebar() {
  ...
  const logoUrl = typeof localStorage !== "undefined" ? localStorage.getItem("pharmacy_logo") : null;
  const pharmacyName = typeof localStorage !== "undefined" ? (localStorage.getItem("pharmacy_name") || "منظومة") : "منظومة";
    ...
    <Sidebar side="right" className="border-l-0 border-r border-sidebar-border" variant="inset">
      <SidebarHeader className="h-16 flex items-center justify-center border-b border-sidebar-border px-4 py-2">
        <div className="flex items-center gap-2 w-full justify-start text-sidebar-primary">
          ...
          <span className="font-bold text-lg text-sidebar-foreground">{pharmacyName}</span>
        </div>
      </SidebarHeader>
      <SidebarContent>
